...
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.AfterHeadBody;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.BeforeHtmlToHead;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InBodyStartToHead;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InCaptionIgnore;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InCellCol;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InCellNames;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InHeadEmpty;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InHeadNoScriptHead;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InHeadNoscriptIgnore;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InHeadRaw;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InRowIgnore;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InRowMissing;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InSelecTableEnd;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InSelectEnd;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableAddBody;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableBodyEndIgnore;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableBodyExit;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableEndErr;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableEndIgnore;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableFoster;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableToBody;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.InTableToHead;
...
import com.mt.lamdatrack.Rate.nodes.DocumentType;
import com.mt.lamdatrack.Rate.nodes.Element;
import com.mt.lamdatrack.Rate.nodes.Node;

import java.util.ArrayList;

import static com.mt.lamdatrack.Rate.internal.StringUtil.inSorted;
import static com.mt.lamdatrack.Rate.parser.HtmlTreeBuilderState.Constants.*;
...
enum HtmlTreeBuilderState {
    ...
    InHeadNoscript {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isDoctype()) {
                ...
                return anythingElse(t, tb);
            } else if ((t.isStartTag() && inSorted(t.asStartTag().normalName(), Constants.InHeadNoscriptIgnore)) || t.isEndTag()) {
                tb.error(this);
                return false;
            } else {
                return anythingElse(t, tb);
    ...
    AfterHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                if (inSorted(t.asEndTag().normalName(), Constants.AfterHeadBody)) {
                    anythingElse(t, tb);
                } else {
                    tb.error(this);
                    return false;
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isEndTag() && t.asEndTag().normalName().equals("caption")) {
                ...
                if (!tb.inTableScope(name)) {
                    ...
                    t.isStartTag() && inSorted(t.asStartTag().normalName(), Constants.InCellCol) ||
                ...
                if (processed)
                    return tb.process(t);
            } else if (t.isEndTag() && inSorted(t.asEndTag().normalName(), Constants.InCaptionIgnore)) {
                tb.error(this);
                return false;
            } else {
                return tb.process(t, InBody);
    ...
    InSelectInTable {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isStartTag() && inSorted(t.asStartTag().normalName(), Constants.InSelecTableEnd)) {
                tb.error(this);
                tb.processEndTag("select");
                return tb.process(t);
            } else if (t.isEndTag() && inSorted(t.asEndTag().normalName(), Constants.InSelecTableEnd )) {
                tb.error(this);
